     ...
     message += `🕐 ${game.time}\n\n`;
        ...
        message += `*Confirmed Players (${game.confirmedPlayers.length}/${game.maxPlayers}):*\n`;
         game.confirmedPlayers.forEach((player, index) => {
           ...
           const dots = ".".repeat(Math.max(1, 14 - label.length));
           if (player.signed_up_at) {
             const signedAt = new Date(player.signed_up_at);
             const datePart = new Intl.DateTimeFormat("en-US", { timeZone: "America/New_York", month: "numeric", day: "numeric" }).format(signedAt);
             const timePart = new Intl.DateTimeFormat("en-US", { timeZone: "America/New_York", hour: "2-digit", minute: "2-digit", hour12: false }).format(signedAt);
             message += `${label}${dots}${datePart} (${timePart})\n`;
           } else {
             message += `${label}\n`;
           }
         });
     ...
     if (openSpots > 0) {
       ...
       if (game.waitlist.length > 0) {
         ...
         game.waitlist.forEach((player, index) => {
           ...
           const dots = ".".repeat(Math.max(1, 14 - label.length));
           if (player.added_at) {
             const addedAt = new Date(player.added_at);
             const datePart = new Intl.DateTimeFormat("en-US", { timeZone: "America/New_York", month: "numeric", day: "numeric" }).format(addedAt);
             const timePart = new Intl.DateTimeFormat("en-US", { timeZone: "America/New_York", hour: "2-digit", minute: "2-digit", hour12: false }).format(addedAt);
             message += `${label}${dots}${datePart} (${timePart})\n`;
           } else {
             message += `${label}\n`;
           }
      ...
      message += `\nIf you need to register / cancel, click 10xx.com`;
